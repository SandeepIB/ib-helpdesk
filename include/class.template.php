...
class EmailTemplateGroup {
    ...
    static $all_names=array(
        ...
        // infobeans added code for closed ticket
        'ticket.closed'=>array(
            'group'=>'a.ticket.user',
            'name'=>/* @trans */ 'Ticket Closed',
            'desc'=>/* @trans */ 'response sent to user, if ticket is closed by Agent',
            'context' => array(
                'ticket', 'signature', 'response', 'staff', 'poster', 'recipient',
            ),
        ),
        //infobeans added code for Reopen Ticket 
        'ticket.reopenclosedticket'=>array(
            'group'=>'b.ticket.staff',
            'name'=>/* @trans */ 'Ticket Reopen',
            'desc'=>/* @trans */ 'response sent to Agent, if ticket is reopen by user',
            'context' => array(
                'ticket', 'recipient', 'message', 'poster','staff','assignee'
            ),
        ),
        //end infobeans added code
    ...
    function getNewMessageAlertMsgTemplate($cStatus='') {
        if($cStatus==3){
            return $this->getMsgTemplate('ticket.reopenclosedticket'); 
        }else{
    ...
    function getReplyMsgTemplate($Tstatus='') {
        if($Tstatus==3){           //modified line by infobeans to add condition for closing ticket where 3 is closed key id
