...
implements RestrictedAccess, Threadable, Searchable {
    ...
    function getCustomSLADueDate($recompute=false) {
        global $cfg;
        ...
        if (($sla = $this->getSLA()) && $sla->isActive()) {
            $schedule = $this->getDept()->getSchedule();
            $tz = new DateTimeZone($cfg->getDbTimezone());
            $dt = new DateTime($this->getReopenDate() ?: $this->getCreateDate(), $tz);
    
            $createdDay = $dt->format('N'); // Get the day of the week for the created date

            // If created on Thursday or Friday, skip to Monday/Tuesday respectively
            if ($createdDay == 4) { // Thursday
                $dt->modify('+2 days'); // Skip Saturday and Sunday
            } elseif ($createdDay == 5) { // Friday
                $dt->modify('+2 days'); // Skip Saturday and Sunday
            } elseif ($createdDay == 6) { // Saturday
                $dt->modify('+1 days'); // Skip Sunday 
            } 
    ...
    function updateEstDueDate($clearOverdue=true) {
        ...
        $this->est_duedate = $this->getCustomSLADueDate(true) ?: null;
    ...
    function getCustomEstDueDate() {
        // Real due date or  sla due date (If ANY)
        return $this->getDueDate() ?: $this->getCustomSLADueDate();
    }
    ...
    function onMessage($message, $autorespond=true, $reopen=true) {
        ...
        if ($email
            ...
            //$email->sendAutoReply($user, $msg['subj'], $msg['body'],null, $options);  // comment added by infobeans to not send email on user reply on ticket
        }
    }
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function postMessage($vars, $origin='', $alerts=true) {
      ...
      if ($recipients && $recipients instanceof MailingList)
        ...
        if ($cfg->alertONNewMessage()
            && ($email = $dept->getAlertEmail())
            && ($tpl = $dept->getTemplate())
            && ($msg = $tpl->getNewMessageAlertMsgTemplate($vars['ct_status']))
        ) {
    ...
    function postReply($vars, &$errors, $alert=true, $claim=true) {
        ...
        if ($email
                && $recipients
                && ($tpl = $dept->getTemplate())
                && ($msg=$tpl->getReplyMsgTemplate($vars['reply_status_id']))) {  //modified line by infobeans passed reply_status_id variable
