...
implements TemplateVariable, Searchable {
    ...
    static function getSLAByTopicId($topic_id) {

        $result = db_query('SELECT * FROM '.TOPIC_TABLE.' WHERE topic_id='.db_input($topic_id));
        ...
        $result_sla = db_query('SELECT * FROM '.SLA_TABLE.' WHERE id='.db_input( $sla_id));
        $sla_data = mysqli_fetch_assoc($result_sla);
        }
        return $sla_data;
    }
