...
implements OAuth2AuthBackend  {
    ...
    static $sign_in_image_url = "/images/google.png";
...
implements OAuth2AuthBackend {
    ...
    private function signIn($attrs) {
        if ($attrs && isset($attrs['username'])) {
            ...
            /* InfoBeans hack */
            if (!empty($attrs['email']) && !strpos($attrs['email'], "infobeans.com")) {
             session_destroy();
             Http::redirect(osTicket::get_base_url().'/login.php?account_error=1');
            }
             /* InfoBeans hack */
    ...
    private function onSignIn() {
        $this->redirectTo($_SESSION['_client']['auth']['dest'] ?: osTicket::get_base_url().'tickets.php');
    }
}
